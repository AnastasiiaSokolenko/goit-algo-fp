...
def build_heap_tree(heap, index=0):
    """
    Будує дерево Node з масиву купи (heap).
    """
    if index >= len(heap):
        return None
...
def generate_color(step, total_steps):
    base_color = [135, 206, 250]  # SkyBlue
    darken_factor = step / total_steps  
    
    # Red: 0 → base_color[0]
    r = int(0 + (base_color[0] - 0) * darken_factor)

    # Green: 50 → base_color[1]
    g = int(50 + (base_color[1] - 50) * darken_factor)
...
def dfs_visualize(root, total_steps):
    """Обхід у глибину з візуалізацією (стек)."""
    colors = {}
    stack = [root]
    visited = set()
    step = 0
    ...
    while stack:
        ...
        if node and node.id not in visited:
            ...
            # додаємо правого, потім лівого (щоб лівий відвідався першим)
            if node.right:
...
def bfs_visualize(root, total_steps):
    """Обхід у ширину з візуалізацією (черга)."""
    colors = {}
    queue = deque([root])
    visited = set()
    step = 0
